...
use crate::utils::{
    bbox_2d, next_value_down_relative, next_value_up_relative, round_to_closest_integer, Vertex2,
    EPS,
};

type Bin = Vec<(Vertex2, f64)>;

/// A simple `VertexClusterer` that samples a range of values into a number of bins.
#[derive(PartialEq)]
pub struct VertexClusterer2 {
    bin_size: f64,
    bins: Vec<Vec<Bin>>,
    weighted: bool,
    ...
    first_bin_interval_start: Vertex2,
    // the interval starts of the "last" bin, i.e. top-right. So this goes from max[0] to max[0] + bin_size (for the y-value repsectively)
    last_bin_interval_start: Vertex2,
    num_bins_x: usize,
    num_bins_y: usize,
}

impl VertexClusterer2 {
    /// Create a `VertexClusterer2` from a list of vertices.
    pub fn new(vertices: Vec<Vertex2>, weights: Option<Vec<f64>>, bin_size: f64) -> Self {
        ...
        let mut sampler = Self {
            bin_size,
            bins: vec![vec![vec![]; num_bins_y.floor() as usize]; num_bins_x.floor() as usize],
            first_bin_interval_start: [min_x, min_y],
            last_bin_interval_start: [last_bin_x_start, last_bin_y_start],
    ...
    pub fn get_bin(&self, x_idx: usize, y_idx: usize) -> Option<&Bin> {
        if x_idx < self.num_bins_x && y_idx < self.num_bins_y {
            let bin = &self.bins[x_idx][y_idx];
            if bin.is_empty() {
                return None;
            }
            Some(&self.bins[x_idx][y_idx])
        } else {
            None
        }
    }
    ...
    pub fn get_bin_mean(&self, x: usize, y: usize) -> Option<(Vertex2, f64)> {
        if let Some(bin) = self.get_bin(x, y) {
            ...
            return Some((
                [sum_x / bin.len() as f64, sum_y / bin.len() as f64],
    ...
    fn put_point_in_bin(&mut self, vertex: Vertex2, weight: f64) {
        ...
        // When the value is slightly to big, e.g. x_idx = 3.0000000000000002, the "as usize" later takes care of thate
        ...
        let (start_x, end_x, start_y, end_y) = self.bin_range(x_idx as usize, y_idx as usize);
        ...
        self.bins[x_idx as usize][y_idx as usize].push((vertex, weight));
    }

    /// Map points to their corresponding bins.
    fn put_points_in_bins(&mut self, vertices: Vec<Vertex2>, weights: Vec<f64>) {
    ...
    pub fn simplify(&self) -> (Vec<Vertex2>, Vec<f64>) {
        ...
        for x_idx in 0..self.num_bins_x {
            for y_idx in 0..self.num_bins_y {
                if let Some((mean_vertex, mean_weight)) = self.get_bin_mean(x_idx, y_idx) {
...
impl fmt::Display for VertexClusterer2 {
...
mod tests {
    ...
    fn validate_sampler(sampler: &VertexClusterer2) {
        // env_logger::init();
        let mut num_validated = 0;
        for x_idx in 0..sampler.num_bins_x {
            for y_idx in 0..sampler.num_bins_y {
                if let Some(bin) = sampler.get_bin(x_idx, y_idx) {
                    let (start_x, end_x, start_y, end_y) = sampler.bin_range(x_idx, y_idx);
